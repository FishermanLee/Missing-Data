...
## 1. Mixture DRO/Model Combination
...
- **扩展1：拓展到按时间划分的分布**：本文注重不同retailer的分布需求，实际上假如同一个运营商有12个月的需求数据，最终需求预测也可以用加权；Non-Stationary，因此也存在Distributional Shift
  $$
...
- **实际上是Regression+Kernel的聚合**：先用Regression确定各local residual distribution，**再根据原有数据分布的相似性进行聚合**。
  ...
  答：可以，**Kernel+Uncertainty等价于Phi-divergence DRO**, 因为mixture DRO相当于降级版的Phi-divergence模糊集
...
如果考虑**Mixed Regression**，则在Regression阶段就要确定权重，那么**对应的loss function会更加复杂**，类似于

- **Linear combination of loss function**: 最终loss function是原来loss function和
...
## Regularization for CSO

**核心思想：将Regularization扩展到CSO部分，CSO+Regularization；**
**Regularization相当于the Lagrangian relaxation of hard constraint **
...
### 2. Cost function Regularization- Fairness
...
### 3. Distribution Distance Regularization
...
考虑采用Entropic Regularize的**Sinkhorn Distance**: **结合不同的统计距离，成为一种距离**

![image-20260109112611326](./26-1.9 会议 Idea 整理.assets/image-20260109112611326.png)

但原问题引**入Entropy Regularization**是为了使整个分布更平滑，而不是为了本文的$\mathbb{P}_{\text{local}}$和$$\mathbb{P}_{\text{global}}$$g更近，**要查清楚Entropic的作用**；可以扩展到Phi-divergence，同样只针对绝对连续分布。

![image-20260109112710431](./26-1.9 会议 Idea 整理.assets/image-20260109112710431.png)

可以考虑只采用Wasserstein距离:

- **定义一个Regularized Wasserstein距离**：Regularization考虑实现分布$\mathbb{P}$和$\text{global distribution}$的距离；
  $$
...
## 4. **Model Misspecification** - Distribution Shift / Noise Data 
...
- **视角 1：本质上都和Robust Satisficing一样**：接受额外的loss作为Regularization，允许misspecification
  ...
  **Misspecification**: **将$\hat{\mathbb{P}}$扩展为模糊集$\mathcal{A}$**即为文中的建模；
  $$
  \min_{x\in\mathcal{X}} \ \{ \mathbb{E}_\mathbb{P}[f(\boldsymbol{x},\tilde{\boldsymbol{z}})] +k\Delta(\mathbb{P},\hat{\mathbb{P}}) \mid \forall\mathbb{P}\in\mathcal{P}_0(\mathcal{Z}) \}
  $$
  **Robust Satisficing**: 不最小化cost，**而是最小化fragility**

  ![image-20260115095043390](./26-1.9 会议 Idea 整理.assets/image-20260115095043390.png)
